...
def protein_graph(sequence, edge_index, esm_embed,protrans_embed):
    ...
    # else:
    data = Data(esm=torch.from_numpy(esm_embed),protrans=torch.from_numpy(protrans_embed), edge_index=edge_index, native_x=seq_code)
    return data
def protein_muti_graph(sequence, edge_index, esm_embed, edge_attr, pos):
    seq_code = aa2idx(sequence)
    seq_code = torch.IntTensor(seq_code)
    edge_index = torch.LongTensor(edge_index)
    data = Data(x=torch.from_numpy(esm_embed), edge_index=edge_index, edge_attr=edge_attr, pos=pos, native_x=seq_code)
    return data
def protein_node2Vec_graph(sequence, edge_index, esm_embed, protrans_embed,node2vec_embed,edge_attr):
    seq_len = len(sequence)
    assert esm_embed.shape[0] == seq_len, f"ESM embed dimension mismatch: {esm_embed.shape[0]} vs {seq_len}"
    assert protrans_embed.shape[0] == seq_len, f"ProTrans embed dimension mismatch: {protrans_embed.shape[0]} vs {seq_len}"
    assert node2vec_embed.shape[0] == seq_len, f"Node2Vec embed dimension mismatch: {node2vec_embed.shape[0]} vs {seq_len}"
    assert edge_attr.shape[0] == edge_index[0].shape[0], f"Edge attribute dimension mismatch: {edge_attr.shape[0]} vs {edge_index[0].shape[0]}"
    seq_code = aa2idx(sequence)
    seq_code = torch.IntTensor(seq_code)
    # print("seq_code dtype:", seq_code.dtype)
    edge_index = torch.LongTensor(edge_index)
    data = Data(esm=torch.from_numpy(esm_embed),protrans=torch.from_numpy(protrans_embed), node2vec=node2vec_embed,edge_index=edge_index, edge_attr=edge_attr,native_x=seq_code)
